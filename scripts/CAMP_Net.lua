...
-- ⚠️  VISIBLE_TO controla quién ve los nodos, líneas y zonas en F10:
--   CAMP_Net.COALITION_BLUE (= 2) → solo pilotos azules (PRODUCCIÓN)
--   -1                            → todos: azules, rojos y espectadores/GM (PRUEBAS)
-- Para probar desde Observer o GM: cambiar VISIBLE_TO = -1
-- Para producción 24/7:            cambiar VISIBLE_TO = CAMP_Net.COALITION_BLUE
...
-- ============================================================
-- BLOQUE 1: NODOS
-- captureFlag < 8200 = capturable; >= 8200 = base azul fija
-- ============================================================

CAMP_Net.NODES = {
  INCIR    = { zone="LN_INCIR",    label="Incirlik",       captureFlag=8101, radius=2000 },
  GAZIAN   = { zone="LN_GAZIAN",   label="Gaziantep",      captureFlag=8102, radius=2000 },
  MINAK    = { zone="LN_MINAKH",   label="Minakh",         captureFlag=8103, radius=2000 },
  KUWEIRES = { zone="LN_KUWEIRES", label="Kuweires",       captureFlag=8104, radius=2000 },
  JIRAH    = { zone="LN_JIRAH",    label="Jirah",          captureFlag=8105, radius=2000 },
  TABQA    = { zone="LN_TABQUA",   label="Tabqa",          captureFlag=8106, radius=2000 },
  ERZOR    = { zone="LN_ERZOR",    label="Deir ez-Zor",    captureFlag=8107, radius=2000 },
  ABU      = { zone="LN_ABUAL",    label="Abu al-Duhur",   captureFlag=8108, radius=2000 },
  BASSEL   = { zone="LN_BASEL",    label="Bassel Al-Assad",captureFlag=8109, radius=2000 },
  HAMA     = { zone="LN_HAMA",     label="Hama",           captureFlag=8110, radius=2000 },
  RENE     = { zone="LN_RENE",     label="Rene",           captureFlag=8111, radius=2000 },
  QUSAYR   = { zone="LN_QUSAYR",   label="Al-Qusayr",      captureFlag=8112, radius=2000 },
  SHAYRAT  = { zone="LN_SHAYRAT",  label="Shayrat",        captureFlag=8113, radius=2000 },
  TIYAS    = { zone="LN_TIYAS",    label="Tiyas",          captureFlag=8114, radius=2000 },
  PALMYRA  = { zone="LN_PALMYRA",  label="Palmyra",        captureFlag=8115, radius=2000 },
  SAYQAL   = { zone="LN_SAYQAL",   label="Sayqal",         captureFlag=8116, radius=2000 },
  NASIRI   = { zone="LN_NASIRI",   label="An Nasiriya",    captureFlag=8117, radius=2000 },
  RAYAK    = { zone="LN_RAYAK",    label="Rayak",          captureFlag=8118, radius=2000 },
  DUMAYR   = { zone="LN_DUMAYR",   label="Al-Dumayr",      captureFlag=8119, radius=2000 },
  MEZZE    = { zone="LN_MEZZE",    label="Mezzeh",         captureFlag=8120, radius=2000 },
  MARJ     = { zone="LN_MARJ",     label="Marj Ruhayyil",  captureFlag=8121, radius=2000 },
  KALK     = { zone="LN_KALK",     label="Khalkhalah",     captureFlag=8122, radius=2000 },
  TALAH    = { zone="LN_THALAH",   label="Tha'lah",        captureFlag=8123, radius=2000 },
  -- Azules fijas
  PAPHOS   = { zone="LN_PAPHOS",   label="Paphos",         captureFlag=8201, radius=2000 },
  AKRO     = { zone="LN_AKRO",     label="Akrotiri",       captureFlag=8202, radius=2000 },
  LARNA    = { zone="LN_LARNA",    label="Larnaca",        captureFlag=8203, radius=2000 },
  KINGS    = { zone="LN_KINGS",    label="Lakatamia",      captureFlag=8204, radius=2000 },
  GECIT    = { zone="LN_GECIT",    label="Gecitkale",      captureFlag=8205, radius=2000 },
  ERKAN    = { zone="LN_ERKAN",    label="Ercan",          captureFlag=8206, radius=2000 },
  CVN73    = { zone="LN_CVN73",    label="CVN-73",         captureFlag=8207, radius=2000 },
  RAMAT    = { zone="LN_RAMAT",    label="Ramat David",    captureFlag=8208, radius=2000 },
  SHEMER   = { zone="LN_SHEMER",   label="Eyn Shemer",     captureFlag=8209, radius=2000 },
  HERZI    = { zone="LN_HERZI",    label="Herzliya",       captureFlag=8210, radius=2000 },
  TELNOF   = { zone="LN_TELNOF",   label="Tel Nof",        captureFlag=8211, radius=2000 },
  HATZOR   = { zone="LN_HATZOR",   label="Hatzor",         captureFlag=8212, radius=2000 },
  TEYMAN   = { zone="LN_TEYMAN",   label="Teyman",         captureFlag=8213, radius=2000 },
  NEVATIM  = { zone="LN_NEVATIM",  label="Nevatim",        captureFlag=8214, radius=2000 },
}
...
function CAMP_Net.Msg(text, seconds)
  trigger.action.outTextForCoalition(CAMP_Net.VISIBLE_TO, text, seconds or 10)
end

function CAMP_Net.RemoveMark(id)
  if id then pcall(function() trigger.action.removeMark(id) end) end
end
...
function CAMP_Net.DrawNode(nodeName)
  ...
  trigger.action.markToCoalition(
    node.markId, node.label.." — "..statusText,
...
function CAMP_Net.DrawAll()
  for i, _ in ipairs(CAMP_Net.EDGES) do CAMP_Net.DrawEdge(i) end
  for nodeName, _ in pairs(CAMP_Net.NODES) do CAMP_Net.DrawNode(nodeName) end
end
...
-- ============================================================
-- DETECCIÓN DE CAPTURA — POLLING OPTIMIZADO
-- 2 llamadas a coalition.getGroups() por ciclo, una por bando.
-- ============================================================
      ...
      if zone then
        ...
        if blueCount > 0 and redCount == 0 then
          if zc.side ~= "blue" then
            zc.side = "blue"; zc.since = now
            env.info("CAMP_Net :: "..nodeName.." → control azul (timer)")
          end
          if not node.captured and (now-zc.since) >= CAMP_Net.CAPTURE_DELAY then
            CAMP_Net.CaptureNode(nodeName)
          end

        elseif redCount > 0 and blueCount == 0 then
          if zc.side ~= "red" then
            zc.side = "red"; zc.since = now
            env.info("CAMP_Net :: "..nodeName.." → control rojo (timer)")
          end
          if node.captured and (now-zc.since) >= CAMP_Net.CAPTURE_DELAY then
            CAMP_Net.ReleaseNode(nodeName)
          end

        elseif blueCount > 0 and redCount > 0 then
          if zc.side ~= "contest" then
            zc.side = "contest"; zc.since = now
            env.info("CAMP_Net :: "..nodeName.." → EN DISPUTA")
          end
          pcall(function()
            local point = CAMP_Net.GetZonePoint(node.zone)
            if point and node.circleId then
              trigger.action.circleToAll(
                CAMP_Net.VISIBLE_TO, node.circleId, point,
                radius, CAMP_Net.COLORS.YELLOW, {1,1,0,0.15}, 2, true, ""
              )
            end
          end)

        else
          if zc.side ~= nil then zc.side=nil; zc.since=now end
        end
...
function CAMP_Net.CheckAllCaptured()
  ...
  CAMP_Net.Msg("RED COMPLETA CAPTURADA. Todas las bases bajo control BLUE.", 25)
